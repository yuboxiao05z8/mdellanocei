<template>
  <div class="addEditCompany">
    <el-dialog center title="Area" :visible.sync="show" :close-on-click-modal="false" :close-on-press-escape="false" width="800px" append-to-body :show-close='false'>
    ...
    addDataFn () {
      this.$refs['form_pro'].validate((valid) => {
        if (valid) {
          ...
          this.$Post(this.$api.saveOrUpdatePndDistrict, this.proForm).then(res => {
            if (res.code == 0) {
              if (this.type === 'edit') {
