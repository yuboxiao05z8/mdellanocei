<template>
  <div class="organization_section">
              ...
              <div class="tbody_item_right">
                <div class="right_item flex opration_column">
                  <div class="opration_column_children">
                    <el-button
                      size="mini"
                      @click="getPermissionLists(item)"
                      :disabled="!item.role.roleId"
                    >{{$t('role.EditPermissions')}}</el-button>
                  </div>
                  ...
                  <div class="data_column_item flex" v-for="(team,i) in item.teams">
                    <div class="data_column_item_left flex">
                      <div class="team_name">
                        <el-select
                          v-if="!team.teamId"
                          size="mini"
                          v-model="item.role.selectTeam"
                          value-key="teamId"
                        >
                          <el-option
                            v-for="teamVal in item.role.selectTeamList"
                            :key="teamVal.teamId"
                    ...
                    <div class="data_column_item_right">
                      <el-button
                        ...
                        :disabled="!team.teamId&&!item.role.selectTeam"
    ...
    <el-dialog
      :title="$t('role.EditPermissions')"
      :visible.sync="dialogVisible"
      width="900px"
      class="roles_dialog"
    >
      <el-row class="dialog_header">
        <el-col :span="12" class="col_text">{{$t('role.Role')}}： {{selectRoleName}}</el-col>
    ...
    <!-- 导航权限弹窗 -->
    <select-nav ref="SelectNav" type="roleId" :receiveObj="sendObj" />
  </div>
</template>
<script>
import customBtn from '@/components/customBtn'
import SelectNav from '@/views/navigation/module/selectNav'
export default {
  components: { customBtn, SelectNav },
  data() {
    return {
      dialogVisible: false,
      roleLists: [],
