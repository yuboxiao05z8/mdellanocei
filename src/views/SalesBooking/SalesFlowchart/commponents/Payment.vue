        ...
        <el-form :model="form" :rules="rules" ref="PaymentForm" label-width="220px">
          ...
          <el-form-item label="Bank" prop="bankName">
            <el-select style="width: 100%" v-model="form.bankName">
              <el-option v-for="(item, index) in bankData" :key="index" :label="item.bankName" :value="item.bankName">
              </el-option>
            </el-select>
          </el-form-item>
          <!-- <el-form-item
          ...
          <el-form-item v-if="form.method !== 'Cash'" label="Cheque No./Transfer Ref." prop="chequeNo">
            <el-input style="width: 100%" v-model="form.chequeNo"></el-input>
          </el-form-item>
          <el-form-item v-if="form.method == 'Cheque'" label="Cheque Date" prop="chequeBankDate">
            <el-date-picker style="width: 100%" v-model="form.chequeBankDate" type="date" value-format="yyyy-MM-dd">
            </el-date-picker>
          </el-form-item>
  ...
  methods: {
    ...
    addDataFn () {
      this.$refs['PaymentForm'].validate((valid) => {
        if (valid) {
          if (this.payerImg && this.payerImg.length === 0) {
          ...
          if (typeof this.editIndex == 'number') {
            // this.$nextTick(() => {
            this.$set(this.Payment.buyerPaymentList, this.editIndex, this.form)
