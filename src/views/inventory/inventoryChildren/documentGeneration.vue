  ...
  <div class="document_generation_wrapper">
        ...
        <el-row>
          <el-col :span="12" class="tab_col">
            <div
              ...
              @click="changeIndex(1)"
            >{{$t('documentGeneration.ImportSite')}}</div>
            <div
              class="tab_div"
              :class="{'active':navIndex == 2}"
              @click="changeIndex(2)"
            >{{$t('documentGeneration.Watermarking')}}</div>
          </el-col>
          <el-col :span="12" class="btn_col">
            ...
            <el-button size="small" :disabled="self == 0" @click="getListData">{{$t('Refresh')}}</el-button>
          </el-col>
        </el-row>
      </div>
    </div>
      ...
      <el-table border style="width: 100%" size="mini" :data="tableList">
        ...
        <el-table-column :label="$t('documentGeneration.AllowGenerate')">
          <template slot-scope="scope">
            ...
            ></el-checkbox>
            <template v-else>
              <i class="el-icon-check" v-if="scope.row.allowGenerate == '0'"></i>
              <i class="el-icon-close" v-else></i>
            </template>
          </template>
        </el-table-column>
        ...
        <el-table-column :label="$t('documentGeneration.NeedWatermark')">
          <template slot-scope="scope">
            ...
            ></el-checkbox>
            <template v-else>
              <i class="el-icon-check" v-if="scope.row.needWatermark == '0'"></i>
              <i class="el-icon-close" v-else></i>
            </template>
          </template>
        </el-table-column>
        <el-table-column :label="$t('documentGeneration.RequiredFor')">
          <template slot-scope="scope">
            <el-select v-model="requiredFor" v-if="scope.$index === tableDataInit" size="mini">
              ...
              <el-option value="Pending Reservation" lable="Pending Reservation"></el-option>
        ...
        <el-table-column :label="$t('userLists.edit')" width="180">
          <template slot-scope="scope">
            <template v-if="scope.$index === tableDataInit">
              ...
              <el-button :disabled="self == 0"  size="mini" plain @click="cancel(scope.row,scope.$index)">{{$t('cancel')}}</el-button>
            </template>
            <template v-else>
              <el-button
                size="mini"
    ...
    <div class="watermarking" v-if="navIndex == 2">
      <div class="prompt_text">
        <el-alert :title="$t('documentGeneration.waterImageCommand')" type="info" :closable="false"></el-alert>
      </div>
      <div class="upload_waterImg">
        <uploader
          ...
          fileType=".png"
          :selfNum="self"
          :btnText="{select:$t('documentGeneration.SelectFile'),import:$t('update')}"
        ></uploader>
        <el-button :disabled="self == 0"  size="mini" @click="downWater">{{$t('documentGeneration.Download')}}</el-button>
        <el-button
          size="mini"
          @click="deleteWater"
          :disabled="self == 0" 
        >{{$t('documentGeneration.DeleteWatermarkImage')}}</el-button>
      </div>
    </div>
  </div>
</template>
